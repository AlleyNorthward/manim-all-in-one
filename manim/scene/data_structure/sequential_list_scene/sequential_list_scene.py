...
"""
    ...
        ...
        中设立好mode,代码端仍需自己写(由于play是不可控的,无法动态的添加和清除,所以想要一了百了,一个函数解决所有,有可能
        实现(比如self.play中添加系列if else),但必然很冗杂,而且很难优化(比如又添加了新的mobject, 需要摆放到哪呢?)),用
        户调用端可以直接根据mode,来选择自己想用的模式.接口固定不变,都是Insert_Sq,避免了重新设立不同的Insert_Sq的方式.
"""
