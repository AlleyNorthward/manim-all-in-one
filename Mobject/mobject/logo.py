from manim import(
    VGroup,SVGMobject, Tex,TexTemplateLibrary, VMobjectFromSVGPath, 
...
"""...
    ...
    @auther 巷北
    @time 2025.9.22
    气死我了.本来上面的都设计好了,svg图没上传github,然后中间这段时间清理一了一下F盘,很自然的都删掉了...幸好还有原图和源代码,但是,原图
    是没有修改的,还需要自己设计一下,并且,之前的距离,位置啥的都专门弄好了,这下又得重新来弄...无语死了........所以说,不论如何,都得保存,
    留个后手...
    ...
    @auther 巷北
    @time 2025.9.23 22:14
    好了,更改完了.增添了一个flip参数,因为设计的时候,想让它中空,弄了弄发现,似乎只能沿着路径向两侧拓展,而不能单独拓展一遍,这就很受限.仔细琢磨了一下,只能弄到目前这样,
    要是width更大,反而显得有些奇怪.用于存在两种模式,会影响组件位置与移动情况,导致内部会有很多信息微调,也只是在当前不为进行微调,感觉设计的不是很合理.如果在外部的话,
    缺少接口访问(似乎还是有接口的...).算了,还是再去更改一下吧.
    ...
    @auther 巷北
    @time 2025.9.29 20:57
    更改文件名 AlgorithmBanner --> logo
    修改了层级,文件地址导入也随之更改.
...
ASSETS_DIR = Path(__file__).resolve().parents[1] / "assets" / "AlgorithmBanner"
...
class AlgorithmBanner(VGroup):
    ...
    def __init__(self, dark_theme: bool = False, isflip: bool = False):
        super().__init__()
        ...
        scale_size = 0.73 if isflip else 0.79
        self.scale(scale_size)

    def _init_A(self, isflip):
        # A = ACreature('A')
        A = SVGMobject(A_PATH)
        ...
        if isflip:
            A.flip()
            A.set_stroke(color = self.font_color, width = 8, opacity = 1)
            A.scale(1.05)
        else:
            A.set_fill(color = self.font_color, opacity = 1)
        return A
    ...
    def create(self, run_time = 2) -> AnimationGroup:
        # todo 如果在场景中先self.add(),后self.play(Create()), 会有显示bug(无法播放)
        # todo 所以一开始不要add,直接Create()即可.
    ...
    def expand(self):
        ...
        if self.isflip:
            move_scale = 11.5 # 这里误加了,返回值输出的move_scale是个元组
            back_scale = 0.8921
        else:
            move_scale = 11
            back_scale = 0.9527
        
        return (move_scale, back_scale)
    ...
    def _expand(self, run_time: float = 2, has_title = True, **anim_kwargs) -> Succession:    
        ...
        # move_parm = 11.5 if self.isflip else 11

        move_scale, back_scale = self.expand()
        A_shape_offset = move_scale * self.scale_factor
        ...
        def slide_back(mob, alpha):
            ...
            # pram = 0.8921 if self.isflip else 0.9527
            shift(alpha * shape_sliding_overshoot * LEFT*back_scale)

            if alpha == 1:
                mob.remove(m_clone)
                mob.add_to_back(mob.shapes)

        slide_animation = UpdateFromAlphaFunc(  # todo这里打算的是,flip的话,就想让Aset_fill,填充满,但是效果差强人意.日后修改
            self,
            slide_and_uncover,
            run_time = run_time * 2 / 4 if has_title else run_time * 2 / 3,
            rate_func = ease_in_out_cubic
        ) if not self.isflip else AnimationGroup(
            self.A.animate.set_fill(color = self.font_color, opacity=1),
            UpdateFromAlphaFunc(
                self,
                slide_and_uncover,
            ),
