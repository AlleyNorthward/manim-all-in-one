...
ASSDIR = Path(__file__).resolve().parent.parent.parent/"assets"/"ACreature"
...
""" ...
    ...
    又有新的理解了,说一下.结合SingleNode的情况,一开始继承自RoundedRectangle,写起来总会很奇怪,因为那种模式,只是为单一对象(单一路径)去描述的
    可是如果又采用组合模式的话,那么想访问最初始的路径,却无法访问,组合拓展对象又拖垮了整体的层级结构,导致后续继承的话访问困难.所以多元素组合模式,
    不应该以某一对象作为继承载体,而是应该以VGroup作为载体,这样就能很容易地访问每个对象.源代码是模仿的3b1b,版本不同,且本人很久未更新,加上当时自己
    理解有限,只能单纯地模仿.不过现在发现问题后,应该做出一定的改变,使得其更适合ce版本.
    仔细分析了一下,发现似乎没有太大毛病.但是本质上,SVGMobject应该是VGroup,而不是VMobject,因为它没有points,只是有submobjects,所以继承自VMobject
    并不合适.不过目前来看,继承空路径和继承SVGMobject区别也不大,没有遇到异常情况.等后续有异常时再说吧.目前来看,各动画部分都正常,也就不做调整了.
"""
