...
"""
    ...
    将get_svg()重命名为_get_svg(),并新定义get_svg(),返回_get_svg()(因为被装饰器修饰后, 无法直接访问mode, 而是通过path获取的.其实是私有方法,不如添加接口.)
    ...
"""
...
class MovingCode(Code):
    ...
    @svg_path("pink_flower", "MovingCode")
    ...
    def get_svg(self, start, mode = 'pink_flower'):
        self.mode = mode
    ...
    def _transform_svg(self, start, svg:SVGMobject, **anim_kwargs):
        new_svg = self.transform_svg(start)
        return Transform(svg, new_svg, **anim_kwargs)
    
    def get_svg_and_rect(self, start, mode):
